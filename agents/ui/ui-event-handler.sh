...
if [[ -z "${PAYLOAD}" ]]; then
	curl -i -X PUT -H "Content-Type:text/plain"  http://127.0.0.1:5000/send/${SERF_USER_EVENT}
else 
	curl -i -X PUT -H "Content-Type:text/plain" -d "${PAYLOAD}" http://127.0.0.1:5000/send/${SERF_USER_EVENT}
fi
