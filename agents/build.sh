...
if [ "$1" != "serf-base" ] && [ "$1" != "skynet" ] && [ "$1" != "ui" ] && [ "$1" != "resman" ] && [ "$1" != "database" ] && [ "$1" != "webserver" ] && [ "$1" != "loadbalancer" ] && [ "$1" != "python-webapp" ]
  then
    echo "Which image do you want to build?" 
    echo "build <serf-base/skynet/ui/resman/database/webserver/loadbalancer/python-webapp>"
    exit 1;
fi
